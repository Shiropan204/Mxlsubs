import { Episode } from '../../types';

/**
 * Equal Love Kokokko!! (NTV)
 * https://www.ntv.co.jp/equal-love/
 */

const THUMBNAIL = 'https://www.ntv.co.jp/equal-love/images/taxy5s5k7h9amf0u11x2io9mjv8iw3.jpg';

export const equalLoveKokokko: Episode[] = [
  {
    id: 'equallove-kokokko-ep1',
    title: 'Equal Love Kokokko!! Episode 1',
    date: '2026-07-04',
    type: 'Variety',
    series: 'equal-love-kokokko',
    members: [],
    tags: ['variety', 'equal-love'],
    subtitles: [],
    servers: [
      { id: 'drive', name: 'Google Drive', videoId: '1mrB_IrU6uO-ta-6Z7xGWJg_NuSg14chN' },
      { id: 'dailymotion', name: 'Dailymotion', videoId: 'k5c6rD2vwW3CR4HlhIm' },
    ],
    subtitleTracks: [],
    subtitleTags: ['Indo', 'Eng'],
    thumbnailUrl: THUMBNAIL,
  },
  {
    id: 'equallove-kokokko-ep2',
    title: 'Equal Love Kokokko!! Episode 2',
    date: '2026-07-07',
    type: 'Variety',
    series: 'equal-love-kokokko',
    members: [],
    tags: ['variety', 'equal-love'],
    subtitles: [],
    servers: [
      { id: 'dailymotion', name: 'Dailymotion', videoId: 'k5AKaNUtfJgcH0HlhIC' },
    ],
    subtitleTracks: [],
    subtitleTags: ['Indo'],
    thumbnailUrl: THUMBNAIL,
  },
  {
    id: 'equallove-kokokko-ep3',
    title: 'Equal Love Kokokko!! Episode 3',
    date: '2026-07-09',
    type: 'Variety',
    series: 'equal-love-kokokko',
    members: [],
    tags: ['variety', 'equal-love'],
    subtitles: [],
    servers: [
      { id: 'dailymotion', name: 'Dailymotion', videoId: 'k3kLxmCLlTeQoAHlhIq' },
    ],
    subtitleTracks: [],
    subtitleTags: ['Eng'],
    thumbnailUrl: THUMBNAIL,
  },
  {
    id: 'equallove-kokokko-ep4',
    title: 'Equal Love Kokokko!! Episode 4',
    date: '2026-07-09',
    type: 'Variety',
    series: 'equal-love-kokokko',
    members: [],
    tags: ['variety', 'equal-love'],
    subtitles: [],
    servers: [
      { id: 'dailymotion', name: 'Dailymotion', videoId: 'kg55QIWVKhrJsAHlhIu' },
    ],
    subtitleTracks: [],
    subtitleTags: ['Eng'],
    thumbnailUrl: THUMBNAIL,
  },
    {
      id: 'equallove-kokokko-ep5',
      title: 'Equal Love Kokokko!! Episode 5',
      date: '2026-07-09',
      type: 'Variety',
      series: 'equal-love-kokokko',
      members: [],
      tags: ['variety', 'equal-love'],
      subtitles: [],
      servers: [
        { id: 'dailymotion', name: 'Dailymotion', videoId: 'k2rC93xCPInPZjHlhIy' },
      ],
      subtitleTracks: [],
      subtitleTags: ['Eng'],
      thumbnailUrl: THUMBNAIL,
    },
    {
      id: 'equallove-kokokko-ep6',
      title: 'Equal Love Kokokko!! Episode 6',
      date: '2026-07-15',
      type: 'Variety',
      series: 'equal-love-kokokko',
      members: [],
      tags: ['variety', 'equal-love'],
      subtitles: [],
      servers: [
        { id: 'dailymotion', name: 'Dailymotion', videoId: 'k4fONnuldIJ1o1HlhIO' },
      ],
      subtitleTracks: [],
      subtitleTags: ['Eng'],
      thumbnailUrl: THUMBNAIL,
    },
    {
      id: 'equallove-kokokko-ep8',
      title: 'Equal Love Kokokko!! Episode 8',
      date: '2026-07-15',
      type: 'Variety',
      series: 'equal-love-kokokko',
      members: [],
      tags: ['variety', 'equal-love'],
      subtitles: [],
      servers: [
        { id: 'dailymotion', name: 'Dailymotion', videoId: 'k4bdnTaWQncYAGHlhIK' },
      ],
      subtitleTracks: [],
      subtitleTags: ['Indo', 'Eng'],
      thumbnailUrl: THUMBNAIL,
    },
];
