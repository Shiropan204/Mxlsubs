              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {subtitleTracks.length > 0 ? (
                  subtitleTracks.map(track => (
                    ...
                    >
                      <Download size={15} /> <span className="hidden sm:inline">Download {track.label}</span><span className="sm:hidden">{track.label}</span>
                    </a>
                  ))
                ) : (
                  <button className="flex items-center justify-center gap-2 px-4 py-2.5 bg-bg-surface hover:bg-border-subtle rounded-xl text-sm font-semibold transition-all duration-200 flex-1 sm:flex-none border border-border-subtle hover:border-brand/30 hover:text-brand hover:shadow-md hover:shadow-brand/5 cursor-not-allowed opacity-50" disabled>
                    <Download size={15} /> <span className="hidden sm:inline">No Subtitles</span><span className="sm:hidden">None</span>
                  </button>
                )}
